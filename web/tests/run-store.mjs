...
const filePath = join(__dirname, "store", "game-store.test.ts");
...
proc.on("close", (code) => {
  if (code === 0) {
    ...
    console.log(`✓ game-store.test.ts: ${lastLine}`);
    console.log(JSON.stringify({ ok: true, passed: 1, failed: 0 }, null, 2));
    process.exit(0);
  }
  console.error("✗ game-store.test.ts");
