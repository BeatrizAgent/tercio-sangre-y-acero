// Runs backend API/CORS contract checks using tsx.
...
const filePath = join(__dirname, "api", "backend-contract.test.ts");
...
proc.on("close", (code) => {
  if (code === 0) {
    ...
    console.log(`backend-contract.test.ts: ${lastLine}`);
    console.log(JSON.stringify({ ok: true, passed: 1, failed: 0 }, null, 2));
    process.exit(0);
  }
  console.error("backend-contract.test.ts");
