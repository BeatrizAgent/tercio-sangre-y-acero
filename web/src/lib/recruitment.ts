import type { CharacterDefinition, GameState, Soldier } from "./types";

export interface RecruitmentCost {
  coins?: number;
  honor?: number;
  reputation?: number;
}

export interface RecruitmentCandidate {
  id: string;
  hook: string;
  cost: RecruitmentCost;
  character: CharacterDefinition;
}

const emptyEquipment: CharacterDefinition["equipment"] = {
  head: null,
  body: null,
  mainHand: null,
  offHand: null,
  firearm: null,
  accessory: null,
  boots: null,
  consumable: null,
};

export const recruitmentCandidates: readonly RecruitmentCandidate[] = [
  {
    id: "tomas_de_orduna",
    hook: "Pide paga corta y sitio lejos del tambor.",
    cost: { coins: 18 },
    character: {
      id: "tomas_de_orduna",
      name: "Tomas de Orduna",
      role: "Piquero",
      rank: "bisono",
      portraitAssetId: "diego_de_arce_portrait",
      tercioAssetId: "tercio_001",
      emotionAssetId: "tercio_emocion_001",
      formationSlot: "banquillo",
      fatigue: 6,
      stats: {
        pike: 3,
        sword: 1,
        arquebus: 0,
        discipline: 2,
        vigor: 3,
        cunning: 1,
        command: 0,
      },
      equipment: { ...emptyEquipment, mainHand: "common_pike_001" },
    },
  },
  {
    id: "hernan_de_ula",
    hook: "Veterano seco. Solo sirve a capitan con honra visible.",
    cost: { honor: 2 },
    character: {
      id: "hernan_de_ula",
      name: "Hernan de Ula",
      role: "Cabo viejo",
      rank: "soldado_viejo",
      portraitAssetId: "diego_arrodillado_con_pica",
      tercioAssetId: "tercio_015",
      emotionAssetId: "tercio_emocion_015",
      formationSlot: "banquillo",
      fatigue: 14,
      stats: {
        pike: 5,
        sword: 3,
        arquebus: 1,
        discipline: 5,
        vigor: 4,
        cunning: 2,
        command: 2,
      },
      equipment: { ...emptyEquipment, mainHand: "common_pike_010" },
    },
  },
  {
    id: "beltran_de_rojas",
    hook: "Viene por tu fama, no por tu bolsa.",
    cost: { reputation: 4 },
    character: {
      id: "beltran_de_rojas",
      name: "Beltran de Rojas",
      role: "Espadachin",
      rank: "soldado",
      portraitAssetId: "diego_piquero_tres_cuartos",
      tercioAssetId: "tercio_010",
      emotionAssetId: "tercio_emocion_010",
      formationSlot: "banquillo",
      fatigue: 9,
      stats: {
        pike: 2,
        sword: 5,
        arquebus: 1,
        discipline: 3,
        vigor: 4,
        cunning: 3,
        command: 1,
      },
      equipment: { ...emptyEquipment, mainHand: "common_sword_004" },
    },
  },
  {
    id: "mateo_de_breda",
    hook: "Arcabucero con polvora propia y deudas ajenas.",
    cost: { coins: 24, reputation: 1 },
    character: {
      id: "mateo_de_breda",
      name: "Mateo de Breda",
      role: "Arcabucero",
      rank: "soldado",
      portraitAssetId: "sergeant_instructor_portrait",
      tercioAssetId: "tercio_007",
      emotionAssetId: "tercio_emocion_007",
      formationSlot: "banquillo",
      fatigue: 11,
      stats: {
        pike: 1,
        sword: 2,
        arquebus: 6,
        discipline: 3,
        vigor: 2,
        cunning: 4,
        command: 1,
      },
      equipment: { ...emptyEquipment, firearm: "uncommon_arquebus_021" },
    },
  },
  {
    id: "julian_de_napoles",
    hook: "Buen ojo para vendas, agua y malas noticias.",
    cost: { coins: 12, honor: 1 },
    character: {
      id: "julian_de_napoles",
      name: "Julian de Napoles",
      role: "Ayudante",
      rank: "bisono",
      portraitAssetId: "field_surgeon_portrait",
      tercioAssetId: "tercio_003",
      emotionAssetId: "tercio_emocion_003",
      formationSlot: "banquillo",
      fatigue: 4,
      stats: {
        pike: 1,
        sword: 1,
        arquebus: 1,
        discipline: 4,
        vigor: 2,
        cunning: 4,
        command: 0,
      },
      equipment: { ...emptyEquipment, consumable: "objeto_002" },
    },
  },
  {
    id: "rodrigo_de_soria",
    hook: "Quiere soldada, respeto y fila lejos del barro hondo.",
    cost: { coins: 36, honor: 3, reputation: 2 },
    character: {
      id: "rodrigo_de_soria",
      name: "Rodrigo de Soria",
      role: "Sargento",
      rank: "sargento",
      portraitAssetId: "sergeant_instructor_portrait",
      tercioAssetId: "tercio_007",
      emotionAssetId: "tercio_emocion_007",
      formationSlot: "banquillo",
      fatigue: 10,
      stats: {
        pike: 4,
        sword: 4,
        arquebus: 2,
        discipline: 6,
        vigor: 4,
        cunning: 3,
        command: 5,
      },
      equipment: { ...emptyEquipment, mainHand: "common_sword_003" },
    },
  },
] as const;

export function getRecruitmentCandidate(candidateId: string) {
  return recruitmentCandidates.find((candidate) => candidate.id === candidateId);
}

export function canRecruitCandidate(soldier: Soldier, candidate: RecruitmentCandidate) {
  const missing: string[] = [];
  if ((candidate.cost.coins ?? 0) > soldier.coins) missing.push("dinero");
  if ((candidate.cost.honor ?? 0) > soldier.honor) missing.push("honor");
  if ((candidate.cost.reputation ?? 0) > soldier.reputation) missing.push("fama");

  if (missing.length > 0) {
    return {
      ok: false,
      message: `Falta ${missing.join(", ")}.`,
    };
  }

  return { ok: true, message: "Puede reclutarse." };
}

export function recruitCandidateIntoState(
  state: GameState,
  candidateId: string,
): { ok: boolean; message: string; state: GameState } {
  const candidate = getRecruitmentCandidate(candidateId);
  if (!candidate) {
    return { ok: false, message: "Recluta no encontrado.", state };
  }
  if (state.characters.some((character) => character.id === candidate.character.id)) {
    return { ok: false, message: "Ya esta en tu tercio.", state };
  }

  const allowed = canRecruitCandidate(state.soldier, candidate);
  if (!allowed.ok) return { ...allowed, state };

  return {
    ok: true,
    message: `${candidate.character.name} entra en el tercio.`,
    state: {
      ...state,
      soldier: {
        ...state.soldier,
        coins: state.soldier.coins - (candidate.cost.coins ?? 0),
      },
      characters: [
        ...state.characters,
        {
          ...candidate.character,
          formationSlot: "banquillo" as const,
          stats: { ...candidate.character.stats },
          equipment: { ...candidate.character.equipment },
          unlocked: true,
        },
      ],
    },
  };
}
